...
function MapNode({ status, stars, pulseAnim, goldAnim, levelNum, ayahFrom, ayahTo, isFetching }: {
  ...
  // Real gate — previous level not completed. Shows a lock icon instead of
  // the level number; the first level of every surah never reaches this
  // branch (it's always 'pending' or resolved from the backend), so it
  // always shows its number and stays open.
  return (
    <View style={S.nodeWrapper}>
...
const S = StyleSheet.create({
  ...
  lockIcon:      { fontSize: 18, opacity: 0.75 },
